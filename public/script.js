...
const REPLIT_DOMAIN = '520c2ff8-c1a2-49ef-8a32-a59e0ea22cce-00-2ljs0gasz19uv.sisko.replit.dev';
const WS_SERVERS = [
...
const HEARTBEAT_INTERVAL = 15000; // Reduced from 30 seconds to 15 seconds
...
function addSystemMessage(message) {
    ...
    // Auto-delete system messages after 3 seconds
    setTimeout(() => {
...
function addVideoSystemMessage(message, options = {}) {
    ...
    // Auto-delete ALL system messages after 3 seconds (unless explicitly disabled)
...
function startLongPress(messageEl, event) {
...
function showMessageContextMenu(messageEl, event) {
    // Remove any existing context menus
    ...
    // Remove menu when clicking elsewhere
    setTimeout(() => {
        document.addEventListener('click', function menuClickHandler() {
            menu.remove();
            document.removeEventListener('click', menuClickHandler);
        });
    }, 100);
    
    console.log('📱 Message context menu shown');
}
...
function handleVideoSkip() {
    ...
    // Set flag to indicate this is a user-initiated skip
    isUserSkipping = true;
    ...
    // SKIP-SPECIFIC CLEANUP: Only cleanup remote, preserve local stream
    skipVideoCleanup();

    // Reset UI for skip (without connecting animation)
    resetVideoUIForSkip();
...
// Skip-specific cleanup function that ONLY cleans remote connections
function skipVideoCleanup() {
    ...
    // Clean up ONLY remote video (keep local stream running)
    if (remoteVideo && remoteVideo.srcObject) {
        ...
        remoteVideo.load();
        console.log('✅ Remote video cleared, local stream preserved');
    }
    
    // Stop ONLY peer connection (not local stream)
    if (peerConnection) {
    ...
    console.log('✅ Skip cleanup completed - LOCAL STREAM PRESERVED ✅');
}
...
function proceedWithVideoSkip() {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            setConnectButtonDisabled(false);
            console.log('✅ Video skip flag cleared - ready for normal stranger disconnect handling');
        }, 100);
    }, 200);
...
function resetVideoUIForSkip() {
    console.log('🎨 Resetting video UI for skip (preserving stranger preview)...');

    // Reset button state to "New" 
    const label = videoConnectBtn?.querySelector('span');
    if (label) {
        label.textContent = "New";
    ...
    // The stranger preview should remain visible until new connection
    console.log('✅ Stranger preview window preserved during skip');
    ...
    console.log('✅ Skip UI reset completed - stranger preview preserved');
}
...
function validateMediaFile(file) {
    ...
    if (MEDIA_VALIDATION.ALLOWED_IMAGE_TYPES.includes(file.type)) {
        ...
        errors.push(`Invalid file type: ${file.type}. Only PNG, JPEG, WebP images and MP4, WebM, MOV, AVI videos are allowed.`);
        return { isValid: false, errors, fileType: null };
    }
