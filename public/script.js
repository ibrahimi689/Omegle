...
function handleVideoSkip() {
    ...
    // Send leave message to server (keep WebSocket alive for new partner)
    if (socket && socket.readyState === WebSocket.OPEN) {
        socket.send(JSON.stringify({ type: 'leave' }));
        console.log('📤 Sent leave message to server (keeping connection alive)');
        
        // Start looking for new partner immediately
        setTimeout(() => {
            addVideoSystemMessage("Looking for people online");
            const interests = interestsInput ? interestsInput.value.split(',').map(i => i.trim()).filter(i => i) : [];
            socket.send(JSON.stringify({
        ...
        }, 200); // Reduced from 500ms to 200ms for faster connection
    }
