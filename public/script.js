...
function addVideoSystemMessage(message, options = {}) {
    ...
    // Check if this message should auto-dismiss
    const shouldAutoDismiss = options.autoDismiss || TRANSIENT_VIDEO_SUCCESS.some(t => message.includes(t));
...
function handleReceivedMessage(data) {
    ...
    if (data.mediaType && data.mediaURL) {
        // Extract filename and size from the message if available
...
// New function specifically for safely exiting video chat while preserving option to restart
function exitVideoChat() {
    console.log('🚪 Exiting video chat safely...');
    
    // Add user disconnect message
    addVideoSystemMessage("You have left the video chat.");
    
    // Stop peer connection and clean up remote video only
    stopPeerConnection();
    cleanupRemoteVideo();
    
    // Perform partial cleanup - preserve local video stream for quick restart
    cleanupWebRTCConnection('video_exit');
    ...
    console.log('✅ Video chat exit completed - local stream preserved for quick restart');
}
...
const MEDIA_VALIDATION = {
    ...
    ALLOWED_VIDEO_TYPES: [], // Videos are now rejected for security
    MAX_IMAGE_SIZE: 5 * 1024 * 1024, // 5MB for images
    MAX_VIDEO_SIZE: 0, // Videos rejected
    MAX_WIDTH: 1920,
    MAX_HEIGHT: 1080,
...
function validateMediaFile(file) {
    ...
    if (MEDIA_VALIDATION.ALLOWED_IMAGE_TYPES.includes(file.type)) {
        ...
        errors.push(`Invalid file type: ${file.type}. Only PNG, JPEG, and WebP images are allowed.`);
        return { isValid: false, errors, fileType: null };
    }
    ...
    if (fileType === 'image' && file.size > MEDIA_VALIDATION.MAX_IMAGE_SIZE) {
        ...
        errors.push('Video files are not allowed for security reasons');
    }
...
function handlePhotoUpload(event) {
    ...
    try {
        ...
        fileToBase64(file, true).then(base64DataURL => {
            ...
            // Send compressed base64 data URL to recipient
            if (socket?.readyState === WebSocket.OPEN) {
                socket.send(JSON.stringify({
                    type: 'message',
                    message: `📷 Photo: ${fileName} (${compressedSize}MB)`,
                    mediaType: 'image',
...
function handleVideoUpload(event) {
    ...
    // Videos are now rejected for security reasons
    alert('Video files are not allowed for security reasons. Please use image files only (PNG, JPEG, WebP).');
    event.target.value = '';
    return;
}
...
function handleMediaValidationError(data) {
    ...
    hideUserFeedback();
    showUserFeedback('Server rejected media: ' + data.message, 'error');
}
