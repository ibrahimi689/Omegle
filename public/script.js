...
async function startVideoChat() {
    ...
    // Show local circle indicator immediately (without connecting animation)
    const strangerVideo = document.querySelector('.stranger-video');
    if (strangerVideo) {
        // Remove connecting class to show the default circle indicator
        strangerVideo.classList.remove('connecting');
        console.log('✅ Local circle indicator displayed immediately');
    }
    ...
    // Full WebRTC setup with local and remote video
    try {
...
function handleWebSocketMessage(data) {
    ...
    switch (data.type) {
        ...
        case 'waiting':
            ...
            // Add connecting animation for video chat when WebSocket is searching
            if (currentChatType === 'video') {
                const strangerVideo = document.querySelector('.stranger-video');
                const connectingCircle = document.getElementById('connectingCircle');
                
                // Show connecting animation only when WebSocket is actively searching
                if (strangerVideo) {
                    strangerVideo.classList.add('connecting');
                    console.log('🔍 WebSocket searching - showing connecting animation');
                }
