...
function cleanupConnections(fullCleanup = false) {
    ...
    if (fullCleanup) {
        try {
            // NSFW detector temporarily disabled
...
async function startVideoChat() {
    ...
    // FIX: Do not show spinner proactively. It will now be shown only when the server sends a 'waiting' message.
    // This prevents the spinner from appearing prematurely.
    const strangerVideo = document.querySelector('.stranger-video');
    if (strangerVideo) {
        strangerVideo.classList.remove('connecting');
    }
    const spinnerEl = document.getElementById('connectingCircle');
    if (spinnerEl) {
        spinnerEl.classList.add('hidden');
    }
    console.log('🎬 Video chat started. Waiting for server "waiting" signal to show spinner.');
    ...
    try {
        ...
        console.error('❌ Local camera setup failed:', error);
        addVideoSystemMessage("Please check your camera permissions");
        
        setTimeout(() => {
            const tryAgainBtn = document.createElement('button');
            tryAgainBtn.textContent = 'Try Again';
            tryAgainBtn.onclick = () => startVideoChat();
...
// Helper functions for video setup
async function ensureVideoPlayback(videoElement) {
    try {
        await videoElement.play();
        console.log('✅ Video playback started successfully');
    } catch (playError) {
        console.warn('Video autoplay issue:', playError.name);
        if (playError.name === 'NotAllowedError') {
            addTapToPlayButton(videoElement, 'local');
        }
    }
}

function applyLocalVideoStyling() {
    ...
    if (isMobileDevice()) {
        // Mobile: float inside stranger video container
        const strangerVideoContainer = document.querySelector('.stranger-video');
        if (strangerVideoContainer && !strangerVideoContainer.contains(localVideo)) {
            strangerVideoContainer.appendChild(localVideo);
        }
        ...
        // Desktop: normal user video window
        const userVideo = document.querySelector('.user-video');
        if (userVideo && !userVideo.contains(localVideo)) {
            userVideo.appendChild(localVideo);
        }
...
function connectWebSocket() {
    // If the socket is already OPEN, proactively send a fresh join for the current chat
    if (socket && socket.readyState === WebSocket.OPEN) {
        console.log('WebSocket already open - sending fresh join for current chat');
        ...
        try {
            ...
            console.error('Failed to send join on already-open socket:', err);
        }
        return;
    }

    // If the socket is CONNECTING, onopen handler will send the join
    if (socket && socket.readyState === WebSocket.CONNECTING) {
        console.log('WebSocket is connecting - join will be sent on open');
        return;
    }
...
function handleWebSocketMessage(data) {
    ...
    switch (data.type) {
        ...
        case 'waiting':
            ...
            // FIX: Spinner is now shown only on server 'waiting' signal.
            // This uses a class-based approach to respect CSS `!important`.
            if (currentChatType === 'video') {
                ...
                if (connectingCircle) {
                    // Use classList.remove to show the spinner, respecting CSS !important.
                    connectingCircle.classList.remove('hidden');
                    console.log('🔌 Server is waiting - showing spinner via class removal.');
                }
            }
            break;
        case 'message':
...
// Helper function to create RTCPeerConnection with basic event handlers
function createPeerConnection() {
    ...
    // Set up ontrack event handler for receiving remote media
    pc.ontrack = (event) => {
    ...
    // Set up onicecandidate event handler for ICE candidate exchange
    ...
    console.log('✅ RTCPeerConnection created with event handlers');
    return pc;
}
...
async function initializeWebRTC(initiator) {
    try {
        ...
        peerConnection.ontrack = (event) => {
            ...
            // FIX: Hide connecting circle and animation when remote track is received using a class-based approach.
            const strangerVideoEl = document.querySelector('.stranger-video');
            if (strangerVideoEl) {
                strangerVideoEl.classList.remove('connecting');
            }
            if (connectingCircle) {
                connectingCircle.classList.add('hidden');
                console.log('✅ Connecting circle hidden and animation stopped - remote track received.');
            }
...
// FIX: Updated test function to use the reliable class-based toggle method.
window.testCircleIndicator = () => {
    console.log('🔍 Testing circle indicator visibility using class-based toggle...');
    const strangerVideo = document.querySelector('.stranger-video');
    const spinner = document.getElementById('connectingCircle');

    if (strangerVideo && spinner) {
        // Toggle the 'hidden' class to show/hide the spinner.
        const isHidden = spinner.classList.toggle('hidden');
        
        // Also toggle the connecting animation class for visual consistency.
        strangerVideo.classList.toggle('connecting', !isHidden);

        console.log(`✅ Spinner visibility toggled. Is now hidden: ${isHidden}`);
    } else {
        console.error('❌ Cannot test spinner: .stranger-video or #connectingCircle element not found.');
    }
};
    ...
    if (connectBtn) {
        connectBtn.addEventListener('click', () => {
            ...
            if (label && label.textContent === "New") {
                ...
                // Ensure a fresh join is sent even if the socket is already open
                const interests = interestsInput ? interestsInput.value.split(',').map(i => i.trim()).filter(i => i) : [];
                if (socket && socket.readyState === WebSocket.OPEN) {
                    try {
                        socket.send(JSON.stringify({
                        ...
                        console.log('📤 Sent manual join for new text partner (socket already open)');
                    } catch (e) {
                        console.error('❌ Failed to send manual join, reconnecting WebSocket...', e);
                        connectWebSocket();
                    }
                } else {
    ...
    if (videoConnectBtn) {
        videoConnectBtn.addEventListener('click', () => {
            ...
            if (label && label.textContent === "New") {
                ...
                // Proactively show connecting visuals
                const strangerVideo = document.querySelector('.stranger-video');
                const spinner = document.getElementById('connectingCircle');
                showConnectingIndicator(); // Use enhanced function


                setConnectButtonDisabled(true);

                // If WebSocket is already open, send a fresh join immediately; otherwise connect
                const interests = interestsInput ? interestsInput.value.split(',').map(i => i.trim()).filter(i => i.length > 0) : [];
                if (socket && socket.readyState === WebSocket.OPEN) {
                    try {
                        socket.send(JSON.stringify({
                            type: 'join',
                        ...
                        console.log('📤 Sent manual join for new video partner (socket already open)');
                    } catch (e) {
                        console.error('❌ Failed to send manual join, reconnecting WebSocket...', e);
                        connectWebSocket();
                    }
...
function handleStrangerConnected(data) {
    ...
    // CRITICAL FIX: Do NOT hide loader here - only hide when remote video actually starts
    // The loader should only be hidden in peerConnection.ontrack when remote stream is received
    console.log('🔗 Stranger connected via WebSocket - starting WebRTC negotiation...');
...
function handleStrangerDisconnected() {
    ...
    // Also hide spinner on disconnect to avoid stale UI
    const _discSpinner = document.getElementById('connectingCircle');
    if (_discSpinner) {
        _discSpinner.classList.add('hidden');
    }
    ...
    // Clean up peer connection only, preserve local video stream
    stopPeerConnection();
}
...
function proceedWithVideoSkip() {
    ...
    // Ensure connecting visuals are shown (idempotent)
    const strangerVideo = document.querySelector('.stranger-video');
    const spinner = document.getElementById('connectingCircle');
    showConnectingIndicator(); // Use enhanced function
...
function resetVideoUIForSkip() {
    ...
    // Proactively show connecting animation and spinner during skip to provide immediate feedback
    const strangerVideoContainer = document.querySelector('.stranger-video');
    const spinner = document.getElementById('connectingCircle');
    if (strangerVideoContainer) {
        strangerVideoContainer.classList.add('connecting');
    }
    if (spinner) {
        spinner.classList.remove('hidden');
    }
    console.log('✅ Stranger video area prepared and connecting animation shown');
...
function performVideoCleanup(reason = 'unknown') {
    ...
    if (remoteVideo) {
        ...
        // Always hide connecting spinner during cleanup to avoid UI leaks
        const _cleanupSpinner = document.getElementById('connectingCircle');
        if (_cleanupSpinner) {
            _cleanupSpinner.classList.add('hidden');
        }
    } else {
        console.log('⚠️ Remote video element not found during cleanup');
    }

    // Comprehensive local video cleanup (preserve local preview unless full cleanup)
    const clearLocal = (reason === 'full_cleanup');
    if (localVideo) {
        ...
        if (clearLocal && localVideo.srcObject) {
            cleanupResults.localStreamState = localVideo.srcObject.active ? 'active' : 'inactive';

            // Only clear local video srcObject during full cleanup
            localVideo.srcObject = null;
            cleanupResults.localVideoCleared = true;
            console.log('✅ Local video srcObject cleared (full cleanup)');
        }

        // Reset or preserve local video element based on cleanup reason
        try {
            if (clearLocal) {
                localVideo.pause();
                localVideo.load();
                console.log('✅ Local video element reset (full cleanup)');
            } else {
                console.log('🟢 Preserving local video element (reason:', reason, ')');
                // Ensure local preview keeps playing
                if (localVideo.srcObject && localVideo.paused) {
                    localVideo.play().catch(e => console.warn('⚠️ Could not auto-play local video after preserve:', e.name));
                }
            }
        } catch (videoError) {
            console.error('❌ Error handling local video element during cleanup:', videoError);
        }
    }
...
// Enhanced function to ensure every user sees circle indicator
function showConnectingIndicator() {
    console.log('🔄 Showing connecting indicator for current user');
    ...
    if (strangerVideo) {
        strangerVideo.classList.add('connecting');
        console.log('✅ Added connecting class to stranger video');
    }
    
    if (connectingCircle) {
        connectingCircle.classList.remove('hidden');
        console.log('✅ Removed hidden class from connecting circle');
    }
    
    // Force CSS reflow to ensure styles apply
    if (strangerVideo) {
        strangerVideo.offsetHeight; // Trigger reflow
    }
}

function hideConnectingIndicator() {
    console.log('🔄 Hiding connecting indicator for current user');
    ...
    if (strangerVideo) {
        strangerVideo.classList.remove('connecting');
        console.log('✅ Removed connecting class from stranger video');
    }
    
    if (connectingCircle) {
        connectingCircle.classList.add('hidden');
        console.log('✅ Added hidden class to connecting circle');
    }
}
